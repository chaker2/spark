...
function PlayPage() {
  ...
  const confirmAvatar = async () => {
    ...
    setJoining(true);
    try {
      const { data, error } = await supabase.from("room_players").insert({ room_id: room.id, username: username.trim(), avatar, client_id: getClientId() }).select("id").single();
      if (error) {
    ...
    } catch (e: any) { toast.error(e.message); setStep("name"); } finally { setJoining(false); }
  };
      ...
      <main className="px-4 py-6 sm:py-10 max-w-2xl mx-auto">
        {!room ? (
          ...
          <div className="rounded-3xl bg-card border border-border shadow-float p-6 sm:p-10 text-center animate-pop-in">
            ...
            <div className="mt-2 font-display text-4xl sm:text-5xl font-bold tracking-[0.25em] bg-clip-text text-transparent bg-primary-gradient">{room.code}</div>
            ...
            return (
              <button key={c.id} onClick={() => onAnswer(c)} disabled={!!myAnswer || expired}
                ...
                <span className="flex items-center gap-3">
                  ...
                  {reveal && <Check className="h-5 w-5" />}
                  {wrong && <X className="h-5 w-5" />}
                </span>
              </button>
            );
          })}
