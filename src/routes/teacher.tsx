...
import { Loader2, LogOut, Plus, Users, Play, X, Copy, Sparkles, SkipForward, Trophy } from "lucide-react";
...
type Player = { id: string; username: string; client_id: string };
type Quiz = { id: string; title: string };
type Question = { id: string; position: number; text: string; time_limit: number; points: number };
...
function TeacherDashboard() {
  ...
  // Load questions when room has quiz_id
  useEffect(() => {
    if (!room?.quiz_id) return;
    (async () => {
      const { data } = await supabase.from("questions").select("id, position, text, time_limit, points").eq("quiz_id", room.quiz_id!).order("position");
      setQuestions((data as Question[]) ?? []);
    })();
  }, [room?.quiz_id]);

  // Subscribe room + players + answers
  useEffect(() => {
    if (!room) return;
    let cancelled = false;
    const loadPlayers = async () => {
      const { data } = await supabase.from("room_players").select("id, username, client_id").eq("room_id", room.id).order("joined_at");
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      setPlayers([]); setScores({}); setShowResults(false);
    ...
    <div className="min-h-screen bg-sky-gradient">
      <header className="sticky top-0 z-50 px-4 pt-4">
        <div className="mx-auto max-w-7xl rounded-3xl bg-card/80 backdrop-blur-md border border-border shadow-soft px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={logout} className="flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:bg-accent transition"><LogOut className="h-4 w-4" /> {t("auth.logout")}</button>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-10 max-w-5xl mx-auto">
        <div className="text-center mb-8 animate-pop-in">
          <h1 className="font-display text-4xl sm:text-5xl font-bold">{t("auth.dashboard")}</h1>
        </div>
        ...
        {!room ? (
          ...
          <div className="grid lg:grid-cols-2 gap-6 animate-pop-in">
            <div className="rounded-3xl bg-card border border-border shadow-float p-8 text-center">
              ...
              <div className="mt-4 font-display text-6xl sm:text-7xl font-bold tracking-[0.25em] bg-clip-text text-transparent bg-primary-gradient">{room.code}</div>
          ...
          <div className="space-y-6 animate-pop-in">
            <div className="rounded-3xl bg-card border border-border shadow-float p-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="font-display text-2xl font-bold text-primary">{room.code}</span>
              </div>
              <h2 className="font-display text-2xl sm:text-3xl font-bold mb-3">{currentQ.text}</h2>
              <div className="grid grid-cols-2 gap-3 mt-6">
                <button onClick={() => setShowResults(true)} className="h-12 rounded-2xl border-2 border-border font-bold hover:bg-accent transition flex items-center justify-center gap-2"><Trophy className="h-4 w-4" /> {t("teacher.showResults")}</button>
            ...
            <Leaderboard sorted={sorted} t={t} />
            <button onClick={closeRoom} className="w-full h-12 rounded-2xl border-2 border-destructive text-destructive font-bold hover:bg-destructive/10 transition">{t("teacher.end")}</button>
          </div>
        ) : (
          <div className="space-y-6 animate-pop-in">
            <div className="rounded-3xl bg-card border border-border shadow-float p-8 text-center">
        ...
        {showResults && <div className="hidden" />}
      </main>
    </div>
  );
}

function PlayersPanel({ players, t }: { players: Player[]; t: any }) {
  return (
    <div className="rounded-3xl bg-card border border-border shadow-float p-8">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-2xl font-bold flex items-center gap-2"><Users className="h-6 w-6 text-primary" /> {t("teacher.players")}</h2>
      ...
      <div className="mt-4 space-y-2 max-h-[420px] overflow-y-auto">
        {players.length === 0 ? <div className="text-center py-10 text-muted-foreground text-sm">{t("teacher.waitingPlayers")}</div> :
          players.map((p, i) => (
            <div key={p.id} style={{ animationDelay: `${i * 40}ms` }} className="flex items-center gap-3 rounded-2xl bg-sky-soft px-4 py-3 animate-pop-in">
...
function Leaderboard({ sorted, t }: { sorted: [string, number][]; t: any }) {
  return (
        ...
        <ul className="space-y-2">
          {sorted.slice(0, 10).map(([name, total], i) => (
            <li key={name} style={{ animationDelay: `${i * 30}ms` }} className="animate-pop-in flex items-center gap-3 rounded-2xl bg-sky-soft px-4 py-3">
              ...
              <div className="h-8 w-8 rounded-lg bg-primary-gradient grid place-items-center text-primary-foreground font-bold text-sm">{name[0]?.toUpperCase()}</div>
